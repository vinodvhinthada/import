...
# ==============================================
# AUTO-REFRESH FUNCTIONALITY FOR MARKET HOURS
# ==============================================

def is_market_open():
    """Check if current time is within market hours (9:00 AM to 3:30 PM IST)"""
...
def run_scheduler():
    ...
    print("🔄 Starting auto-refresh scheduler thread...")
    while True:
...
def trigger_auto_refresh():
    """Manually trigger auto-refresh (useful for testing)"""
    try:
        print("🔄 Manual auto-refresh trigger requested")
        auto_refresh_data()
        return jsonify({
            'status': 'success',
            'message': 'Auto-refresh triggered manually',
            'timestamp': get_ist_time().strftime('%Y-%m-%d %H:%M:%S IST')
        })
    except Exception as e:
        return jsonify({
            'status': 'error',
            'message': f'Error triggering auto-refresh: {str(e)}'
        }), 500
...
def start_auto_refresh_scheduler():
    """Start the auto-refresh scheduler in a background thread"""
    ...
    if scheduler_thread is None or not scheduler_thread.is_alive():
        ...
        print("🚀 Auto-refresh scheduler started successfully!")
    else:
        print("⚠️ Auto-refresh scheduler is already running")
